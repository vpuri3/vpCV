...
\cvsection{Summary}
Builds scalable transformer architectures grounded in numerical analysis and scientific computing.
Developed FLARE (Fast Low-Rank Attention Routing Engine), scaling attention to million-token
regimes on a single GPU via Triton. Background includes numerical PDE methods, distributed training,
and PyTorch systems implementation.
...
\begin{itemize}[leftmargin=1.45em,labelindent=0.45em]
  \item Reduced memory from $\mathcal{O}(N^2)$ to $\mathcal{O}(NM)$. \hfill (Under review)
  \item Implemented Triton + PyTorch modules with reproducible scaling results.
  \item Evaluated on PDE surrogate, NLP, and vision tasks.
  \item arXiv: \href{https://arxiv.org/abs/2508.12594}{2508.12594}, code: \href{https://github.com/vpuri3/FLARE.py}{vpuri3/FLARE.py}.
\end{itemize}

\cvrole{Equation-based PDE modeling with neural fields}{Hybrid data + physics methods}{JCP 2025}
\begin{itemize}[leftmargin=1.45em,labelindent=0.45em]
  \item Neural fields used as nonlinear spatial ansatz in reduced-order modeling.
  \item Physics-based evolution combined with learned spatial representations.
\end{itemize}
...
\cvsection{Teaching}
\cvrole{Carnegie Mellon University}{Teaching Assistant, Numerical Analysis}{Spring 2025}
...
\textbf{Puri, V}. et al., \textit{FLARE: Fast Low-Rank Attention Routing Engine}. arXiv 2025. (Under review)

\textbf{Puri, V}. et al., \textit{SNF-ROM: Projection-based nonlinear reduced order modeling with smooth neural fields}. JCP 2025.

Shankar, V., \textbf{Puri, V.}, et al., \textit{Differentiable physics closure modeling for Burgers' turbulence}. MLST 2023.
...
\cvskill{Machine Learning Systems}{PyTorch, Triton}

\cvskill{Numerical Computing}{Linear systems, finite elements, spectral methods}
